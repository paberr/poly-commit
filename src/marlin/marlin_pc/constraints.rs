...
use ark_ec::{pairing::Pairing, CurveCycle, CurveGroup, Group, PairingFriendlyCycle};
...
type E2Fr<E> = <<E as CurveCycle>::E2 as Group>::ScalarField;
type E2Fq<E> = <<E as CurveCycle>::E2 as CurveGroup>::BaseField;
    ...
    pub fn get_shift_power(
        ...
        if self.constant_allocation {
            ...
            if let Some(shift_power) = shift_power {
                ...
                let supported_bits = E2Fr::<E>::MODULUS_BIT_SIZE;
    ...
    PG: PairingVar<E::Engine2, E2Fq<E>>,
{
    ...
    fn prepare(unprepared: &VerifierKeyVar<E, PG>) -> R1CSResult<Self> {
        let supported_bits = <E2Fr<E> as PrimeField>::MODULUS_BIT_SIZE;
        ...
        T: Borrow<PreparedCommitment<E::Engine2>>,
    {
        ...
        for comm_elem in obj.prepared_comm.0.iter() {
            prepared_comm.push(<PG::G1Var as AllocVar<
                <E::Engine2 as Pairing>::G1,
                E2Fq<E>,
            >>::new_variable(
                ark_relations::ns!(cs, "comm_elem"),
                || {
                    Ok(<<E::Engine2 as Pairing>::G1 as From<
                        <E::Engine2 as Pairing>::G1Affine,
                    >>::from(*comm_elem))
                },
                mode,
...
pub struct BatchLCProofVar<
    E: PairingFriendlyCycle,
    P: DenseUVPolynomial<E2Fr<E>, Point = E2Fr<E>>,
    PG: PairingVar<E::Engine2, E2Fq<E>>,
